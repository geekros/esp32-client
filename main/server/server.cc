...
static esp_err_t http_get_handler(httpd_req_t *req)
{
    ...
    if (strcmp(req->uri, "/") == 0)
    {
        ...
        if (!file)
        {
            ESP_LOGE(TAG, "index.html not found: %s", filepath);
    ...
    // Serve static files for other URIs
...
esp_err_t server_start(server_config_t *config)
{
    ...
    uri_socket.uri = "/socket";
    uri_socket.method = HTTP_GET;
    uri_socket.handler = socket_get_handler;
    uri_socket.is_websocket = true;
    ...
    uri_http.uri = "/*";
    uri_http.method = HTTP_GET;
    uri_http.handler = http_get_handler;
