...
// Constructor
AfeAudioProcessor::AfeAudioProcessor() : afe_data(nullptr)
{
    // Create event group
...
void AfeAudioProcessor::Initialize(AudioCodec *codec_data, int frame_duration_ms, srmodel_list_t *models_list)
{
    ...
    int ref_num = codec->InputReference() ? 1 : 0;

    // Create input format string
    std::string input_format;
    for (int i = 0; i < codec->InputChannels() - ref_num; i++)
    {
    ...
    xTaskCreate(audio_communication_task, "audio_communication", 4096, this, 3, NULL);
}
...
void AfeAudioProcessor::AudioProcessorTask()
{
    ...
    while (true)
    {
        ...
        if (vad_state_change_callback)
        {
            ...
            else if (res->vad_state == VAD_SILENCE && is_speaking)
            {
                is_speaking = false;
